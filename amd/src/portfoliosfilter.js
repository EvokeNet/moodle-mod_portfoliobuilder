...
import GenericFilter from './local/portfoliosfilter/filter';
import Selectors from './local/portfoliosfilter/selectors';
...
/**
 * Initialise the portfolios filter on the element with the given id.
 *
 * @param {String} portfoliosRegionId
 */
export const init = portfoliosRegionId => {
    // Keep a reference to the filterset.
    const filterSet = document.querySelector(`#${portfoliosRegionId}`);
     ...
     */
    const addFilterRow = () => {
        const pendingPromise = new Pending('mod_portfoliobuilder/portfoliosfilter:addFilterRow');

        const rownum = 1 + getFilterRegion().querySelectorAll(Selectors.filter.region).length;
        return Templates.renderForPromise('mod_portfoliobuilder/local/portfoliosfilter/filterrow', {"rownumber": rownum})
            .then(({html, js}) => {
    ...
    const removeAllFilters = () => {
        const pendingPromise = new Pending('mod_portfoliobuilder/portfoliosfilter:setFilterFromConfig');
    ...
    const updateTableFromFilter = () => {
        const pendingPromise = new Pending('mod_portfoliobuilder/portfoliosfilter:updateTableFromFilter');
    ...
    if (initialFilters) {
        const initialFilterPromise = new Pending('mod_portfoliobuilder/portfoliosfilter:setFilterFromConfig');
